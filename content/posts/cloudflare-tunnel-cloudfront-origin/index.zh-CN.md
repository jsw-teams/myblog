...
description: "一次用 Cloudflare Tunnel 给 AWS CloudFront 做私有回源的工程复盘：从 Cloudflare SaaS 绑定踩坑，到统一 origin、CloudFront Function 和 OpenResty 多虚拟主机转发。"
date: "2026-06-23"
updated: "2026-06-23"
...
这次迁移不是一开始就决定用 CloudFront + Cloudflare Tunnel。

最初的问题很具体：`js.gripe` 下面已经有多个 OpenResty 虚拟主机，`blog.js.gripe`、`www.js.gripe`、`dquery.js.gripe` 这类域名各自有不同的静态资源、动态接口、缓存策略和证书行为。原来的 Cloudflare 代理可以让站点跑起来，但当我开始考虑 CloudFront 固定费率套餐时，AWS 又重新进入了候选。

真正让我重新尝试 CloudFront 的原因，是面向中国大陆用户时，AWS 的连通性看起来比 Cloudflare 更值得测试。原来的 Cloudflare 代理可用，但访问体验并不总是稳定；如果 CloudFront 能在这部分用户上表现更好，就值得把它放到前台验证。

CloudFront 以前让我犹豫的是账单模型。pay-as-you-go 下，请求、流量、WAF、日志、边缘函数都分开算。小站点平时可能很便宜，但遇到爬虫、DDoS、缓存命中率下降、动态接口突然变多时，账单不确定性会放大。最近看到 CloudFront flat-rate pricing plans 之后，尝试成本下降了：它把 CDN、WAF、DDoS 保护、DNS、TLS 证书、日志、Serverless Edge Compute 等能力放进月度套餐，并强调 no overage charges。于是 CloudFront Functions 做 host 路由、缓存隔离、cookie 清理这些轻量逻辑，就不再只是“每次调用都要算账”的心理负担。

真正绕了一圈的，是如何让 CloudFront 回源，同时不暴露源站 IP。

顺便也得吐槽一句：到现在 CloudFront 配 origin 仍然不能直接填 IP，必须填一个域名。这个限制在普通公网源站场景里只是有点麻烦，但放到“不暴露源站 IP”的目标下，就变成了架构约束：我们必须先准备一个不会泄露真实源站地址的 origin hostname，再让这个 hostname 通过 Tunnel 回到本机。后面的 `aws-origin.js.gripe`、每域名中间 origin、以及统一 origin 方案，都是围绕这个限制展开的。

这次迁移的大部分排查和实际操作都交给了 Codex：查 AWS/Cloudflare 配置、改 OpenResty、处理 CloudFront Function、部署 Twikoo 前后的缓存问题、最后把部署脚本补成 Cloudflare 和 CloudFront 双清。我这边更多是看最终效果是否符合预期，发现异常再反馈。走完这一轮之后只能感叹一句：Codex 这小子确实有进步了。

<!--more-->

## 第一轮：Cloudflare SaaS 绑定

第一反应是试 Cloudflare SaaS / Custom Hostnames。它看起来很适合“一个服务接多个业务域名”：业务域名挂到一个统一入口，Cloudflare 负责证书和代理，后端服务只管响应。
...
预期是 CloudFront 只看到 `aws.js.gripe`，而 `aws.js.gripe` 后面仍然被 Cloudflare 代理层包住。实际测试却发现，`aws.js.gripe` 直接解析到了 `cf-cdn.js.gripe` 背后的源站 IP。也就是说，原本希望 SaaS 绑定提供的“隐藏源站入口”没有成立，AWS 回源侧仍然可能拿到真实源站地址。

但是这条 DNS 与 SaaS 组合链路没有形成我们想要的安全边界：
...
Cloudflare SaaS 绑定能解决另一类平台托管问题，但在这次迁移里，它没有把这四个边界同时满足。所以第一轮先搁置。
...
- 它不做业务路由。
- 它不保存应用状态。
- 它只把 CloudFront 的 origin 请求带回源站本机端口。
...
如果公网响应里还能看到 `server: cloudflare` 或 `cf-ray`，不能马上判断业务域名开了 Cloudflare 代理。因为 CloudFront 回源到 `aws-origin.js.gripe` 时，origin 段本来就经过 Cloudflare Tunnel，部分响应头可能会从回源段带回来。

## 两种回源组织方式

这里其实有两条路线。

第一条是不使用 CloudFront Function，而是给每个业务域名准备一个固定的中间 origin 名，并在 CloudFront origin 配置里静态写入 `X-Origin-Host`。
...
CloudFront origin 配置里则固定写一条自定义 header：
...
这里 `proxy_set_header Host` 和 `proxy_ssl_name` 必须同时指向 `$origin_requested_host`。前者让 OpenResty 进入正确的业务 `server_name`，后者让本机 HTTPS 回源时使用对应的 SNI。否则请求可能到了 `127.0.0.1:443`，但落进默认 vhost 或证书校验不匹配。
...
它的问题不在技术可行性，而在套餐额度。CloudFront 免费/固定费率套餐里可用的 distribution 数量有限，当时最多只能放三个。如果继续坚持“一个业务域名一个 distribution”，`blog.js.gripe`、`www.js.gripe`、`js.gripe` 三个域名刚好把额度用满，后面再把 `dquery`、`myfiles`、`searchme`、`cf-relay` 这类动态转发或缓存策略不同的域名接进来，就没有余量了。
...
每域名 distribution 的方案是可行的，只是更适合业务域名不超过套餐上限、并且想完全避免 Function 的场景。我们最后决定收敛成统一 origin，是因为 `js.gripe` 下面的业务域名会继续增加，不能把前三个域名之外的服务挡在 CloudFront 之外。
...
关键是 origin request policy 不转发 viewer 的 `Host`。如果直接把 `Host: blog.js.gripe` 转给 `aws-origin.js.gripe`，CloudFront origin 域名、TLS/SNI、Cloudflare Tunnel hostname、OpenResty 业务 host 会混在一起。我们还试过在 CloudFront Function 里改 `request.headers.host`，结果 CloudFront 直接校验失败。CloudFront Functions 不能按这个方式修改 Host。

所以 Function 放在 viewer request 阶段，完整负责这几件事：

- 校验 viewer host 是否在白名单里。
- 清理 viewer cookie，避免无意义的 cookie 进入 origin，也避免动态接口被大 cookie/header 拖出异常响应。
...
这里的重点不是在边缘层做复杂业务逻辑，而是把 CloudFront 不能直接转发的 viewer host 显式转成一个受控 header。`delete request.headers.cookie` 也不是必须项，但对公开站点很实用：静态资源、文章页和大多数只读接口不需要 viewer cookie，提前清掉可以减少回源请求差异，也降低异常大 header 影响 origin 的概率。
...
CloudFront 默认缓存策略现在用 `Managed-CachingOptimized`。动态接口按路径单独建 behavior，需要实时回源的路径可以使用禁用缓存策略。
...
- 响应头里仍能看到 Cloudflare origin 段缓存命中。
...
11. 本机 vhost 的 upstream 端口是否真的在监听。
...
- 数据库密码、后台管理密码、哈希值也不写全。即便是哈希，也按敏感信息处理。

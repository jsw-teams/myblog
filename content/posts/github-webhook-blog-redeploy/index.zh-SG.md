---
title: "使用 GitHub Webhook 实时监测项目变更，并触发网站重新部署"
description: "基于 VPS 上的 /opt/deploy-hooks、GitHub Webhook 签名校验与 OpenResty 反代，让 myweb 与 myblog 的 main 分支变更自动触发双项目重新部署。"
date: "2026-05-10"
updated: "2026-06-02"
...
部署在 VPS 上的静态站点，也可以像平台部署一样实时感知 GitHub 变更。GitHub 在仓库 push 后通知 VPS，VPS 校验签名、快速返回 `202 Accepted`，然后在后台拉替代码、构建产物并 reload OpenResty。
...
也就是说，`myweb` 或 `myblog` 任意一方变更，都会重新建设 `myblog` 和 `myweb`。这样主站写作页、博客文章列表、导航和静态产物会来自同一轮部署结果。
...
`server.mjs` 负责接收 GitHub Webhook、读取原始 body、校验 `X-Hub-Signature-256`、判断事件和仓库分支，然后在后台启动设置中对应的部署命令。
...
真实设置还会包含 `host`、`port` 和 `secret`。`secret` 只保存在服务器上，并和 GitHub Webhook 中填写的值保持一致。`myweb` 和 `myblog` 建议使用同一份密钥，旧的重复入口可以禁用或删除。
...
4. 确认仓库和分支匹配
5. 立即返回 202 Accepted
6. 后台非同步执行 deploy-both.sh
```

GitHub 只需要知道 VPS 已经收到这次变更；真正的建设和 reload 在 VPS 后台继续执行，结果写入日志。
...
`deploy-both.sh` 开始时建立这两个标记文件，部署结束时删除。OpenResty 检测到标记后返回临时 `503` 页面，提示内容正在重构、稍后重新整理即可。

这样部署过程中即使静态目录正在同步，用户看到的也是明确的维护提示，而不是误以为页面遗失。
...
脚本使用 lock file，避免并发部署互相踩踏。建设先发生在临时目录，成功后再同步到对外服务目录，配合 OpenResty 维护页，可以减少部署过程中的 404 窗口。
...
如果 `server.log` 里已经出现 `deploy-accepted`，表示 GitHub 请求已经被 VPS 接收，后续要看部署日志。如果 GitHub timeout 但 `server.log` 没记录，表示请求可能没有到 Node 服务，需要检查 OpenResty、Cloudflare 或 systemd 服务状态。
...
6. VPS 后台执行 deploy-both.sh
7. OpenResty 临时显示维护页
8. 脚本依序建设 myblog 和 myweb，并同步成功产物
9. 清理维护标记、测试并 reload OpenResty
10. 日志写入 /opt/deploy-hooks/logs/
```

这样，VPS 就能实时感知 GitHub 项目变更，并在自己的服务器上套用变更。

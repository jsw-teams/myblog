...
const watchTargets = ["content", "themes", "src", "static", "config.yml", "astro.config.mjs"];
...
function previewClientScript() {
  return `<script>
(() => {
  const id = "siteforge-preview-status";
  const styleId = "siteforge-preview-style";
  ...
  const events = new EventSource("/__siteforge/events");
  events.addEventListener("message", (event) => {
    handleStatus(JSON.parse(event.data), true);
  });
  fetch("/__siteforge/status").then((response) => response.json()).then((data) => {
    handleStatus(data, false);
  }).catch(() => {});
  setInterval(() => {
    fetch("/__siteforge/status", { cache: "no-store" })
...
async function build(reason = "change") {
  ...
  try {
    if (!await buildChangedPage(reason)) {
      ...
      const prebuild = await run(nodeBin, ["src/prebuild.mjs"]);
      if (prebuild.code !== 0) throw new Error(prebuild.output || "prebuild failed");
      const astro = await run(nodeBin, ["node_modules/astro/bin/astro.mjs", "build"]);
...
function createPreviewServer() {
  return createServer(async (request, response) => {
    ...
    if (url.startsWith("/__siteforge/events")) {
    ...
    if (url.startsWith("/__siteforge/status")) {
      response.writeHead(200, {
...
function listen(server, port) {
  ...
  server.listen(port, host, () => {
    ...
    console.log("Polling content, themes, src, static, config.yml, and astro.config.mjs every 10 seconds.");
